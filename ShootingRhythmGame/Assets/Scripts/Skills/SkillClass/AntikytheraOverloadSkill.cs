...
public class AntikytheraOverloadSkill : PlayerSkill
{
    ...
    IEnumerator NormalSkill()
    {
        for (int j = 0; j < 3;)
        {
            ...
            {
                Shoot(transform.position, 4f, i, _skillDamage);
            }
            j++;
    ...
    IEnumerator PowerUpSkill_s(Bullet bullet)
    {
        ...
        Shoot(bullet.transform.position, 8f, angle, _skillDamage);
        Shoot(bullet.transform.position, 8f, angle + 180, _skillDamage);
